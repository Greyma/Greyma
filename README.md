<h1 align="center">Ouahabi Benhenni <sub>aka Greyma</sub></h1>

<p align="center">
  <strong>AI Software Engineer &amp; System Architect</strong> · Co-founder @Badis.ai<br>
  M2 Computer Science, UMBB Algeria · Boumerdès, DZ
</p>

<p align="center">
  <a href="https://github.com/Greyma"><img src="https://img.shields.io/badge/GitHub-Greyma-181717?style=flat&logo=github" /></a>
  <a href="mailto:ouahabi.benhenni@example.com"><img src="https://img.shields.io/badge/Email-contact-0078D4?style=flat&logo=gmail&logoColor=white" /></a>
  <img src="https://img.shields.io/badge/🏆_1st_Place-Algeria--Tunisia_AI_Competition-FAEEDA?style=flat" />
</p>

---

## About

I build production-grade AI systems — from retrieval-augmented generation pipelines and genomic deep learning to real-time multiplayer infrastructure. Currently co-founding **Badis.ai** and finishing my Master's research on SNV pathogenicity prediction with Transformer-based genomic models.

---

## Tech stack

**AI / ML**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat)
![Qdrant](https://img.shields.io/badge/Qdrant-DC244C?style=flat)

**Cloud & Infrastructure**  
![GCP](https://img.shields.io/badge/Google_Cloud-4285F4?style=flat&logo=googlecloud&logoColor=white)
![Vertex AI](https://img.shields.io/badge/Vertex_AI-4285F4?style=flat&logo=googlecloud&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Cloud Run](https://img.shields.io/badge/Cloud_Run-4285F4?style=flat&logo=googlecloud&logoColor=white)

**Web**  
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Socket.io](https://img.shields.io/badge/Socket.io-010101?style=flat&logo=socketdotio&logoColor=white)

---

## Featured projects

### 🔷 RAG-BADIS-AI
> Production RAG system deployed on GCP — Vertex AI (Gemini 2.0 Flash), Qdrant on Cloud Run, LangGraph agentic pipeline, CohereRerank, RAGAS evaluation framework.  
`LangGraph` · `Qdrant` · `Vertex AI` · `text-embedding-004` · `GCP`

---

### 🧬 SNV Pathogenicity Predictor
> Fine-tuned Nucleotide Transformer v2 (500M params) on 500K ClinVar variants.  
> **AUC-ROC 0.987 · PR-AUC 0.983** — strict gene-split OOD validation (BRCA1, BRCA2, TP53, MLH1, MSH2, CFTR, TTN, NF1 held out).  
`Transformers` · `ClinVar` · `NT-v2` · `OOD validation`

---

### 🎮 LoupGarou Multiplayer Platform
> Real-time Werewolf game platform with mediasoup SFU + Socket.io on a self-managed VPS. Sustained real traffic peaks, full reconnection handling, and room lifecycle management.  
`mediasoup` · `Socket.io` · `WebRTC` · `Node.js`

---

### 📚 ActivMath LMS
> Full-stack LMS platform (Flask/Gunicorn + Next.js) deployed at activmath.dz. GitHub Actions CI/CD, Apache reverse proxy, SSL, PM2 process management.  
`Flask` · `Next.js` · `GitHub Actions` · `VPS`

---

## Research

| | |
|---|---|
| **SNV pathogenicity prediction via fine-tuned Nucleotide Transformer** | In preparation · UMBB · 2025 |
| **AI Research Engineer · Institut Pasteur d'Algérie** | Applied deep learning for biomedical sequence analysis |

---

## Recognition

- 🏆 **1st place** — National AI Competition (Algeria-Tunisia)  
- ⚖️ **Judge** — AI Track, FTC Club Hackathon

---

<p align="center">
  <img src="https://streak-stats.demolab.com?user=Greyma&hide_border=true&theme=default" height="140"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Greyma&layout=compact&hide_border=true&theme=default" height="140"/>
</p>
